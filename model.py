...
class RANF_graph(nn.Module):
    def __init__(
        ...
        self.gat1_hrtf = GATConv(256, 64, heads=8, dropout=dropout)
        self.gat2_hrtf = GATConv(64 * 8, 256, heads=8, dropout=dropout)
    ...
    def forward(self, ret_specs_db, ret_itds, tgt_loc, tgt_sidx, ret_sidxs):
        ...
        # x_spec = ret_specs_db[..., :-1].reshape(batch * nretrieval, nch, -1)
        x_spec = ret_specs_db[..., :-1].reshape(batch, nretrieval, -1)
        # graph
        graph_list = []
        for i in range(batch):
            x = x_spec[i]
            node_indices = torch.arange(nretrieval)
            edge_index = torch.combinations(node_indices, with_replacement=False).t()
            graph = Data(x=x, edge_index=edge_index)
            graph_list.append(graph)
        batch_data = Batch.from_data_list(graph_list)
        batch_data = batch_data.cuda()
        x_hrtf, edge_index_hrtf = batch_data.x, batch_data.edge_index
        x_hrtf = self.gat1_hrtf(x_hrtf, edge_index_hrtf)
        x_hrtf = F.elu(x_hrtf)
        x_hrtf = self.gat2_hrtf(x_hrtf, edge_index_hrtf)
        x_spec = x_hrtf.reshape(batch, nretrieval, self.hidden_features, -1)
...
class RANF_Graph2(nn.Module):
    def __init__(
        ...
        self.gat2_hrtf = GATConv(64 * 8, 256, heads=8, dropout=dropout)
        #
        self.gat1_Fusion = GATConv(18 * 128, 128*3, heads=6, dropout=dropout)
        # self.gat2_Fusion = GATConv(128 * 8, 128 * 9, heads=2, dropout=dropout)
    ...
    def forward(self, ret_specs_db, ret_itds, tgt_loc, tgt_sidx, ret_sidxs):
        ...
        #     x = x + self.hidden_blocks[n](x, u=u, v=v)
        x2 = x.shape[2]
        x = x.reshape(batch, nretrieval, -1)

        hf = x.shape[2]  # 每个节点的特征维度
        graph_list = []
        for i in range(batch):
            node_indices = torch.arange(nretrieval)
            edge_index = torch.combinations(node_indices, with_replacement=False).t()  # 生成无向完全图
            graph = Data(x=x[i], edge_index=edge_index)  # 创建 PyG 数据对象
            graph_list.append(graph)
        # 批处理图数据
        batch_data = Batch.from_data_list(graph_list)
        batch_data = batch_data.cuda()
        x_fusion, edge_index_fusion = batch_data.x, batch_data.edge_index
        x_fusion = self.gat1_Fusion(x_fusion, edge_index_fusion)
        x_fusion = F.elu(x_fusion)
        # x_fusion = self.gat2_Fusion(x_fusion, edge_index_fusion)
        x = global_mean_pool(x_fusion, batch_data.batch).reshape(batch, x2, -1)
        # x = torch.mean(x, dim=1, keepdims=False)
        x_itd1, x_itd2, x_spec = x[:, :1, :], x[:, -1:, :], x[:, 1:-1, :]
        # Spectra post-processing
        for n in range(self.spec_hidden_layers):
...
class RANF_Graph3(nn.Module):
    ...
    def forward(self, ret_specs_db, ret_itds, tgt_loc, tgt_sidx, ret_sidxs):
        ...
        x_fusion = self.gat1_Fusion(x_fusion, edge_index_fusion)
        x_fusion = F.elu(x_fusion)
        x_fusion = x_fusion.reshape(batch, nretrieval,x2,-1)
        u, v = self._compute_uv(tgt_onehot, ret_onehot, 1, "alter")
        z = torch.mean(v[..., None, :, :] * x_fusion[..., None], -2)
        z = torch.mean(u[..., None, :, :] * z[..., None, :], -1)
        x = x_fusion + z
        x = torch.mean(x, dim=1, keepdims=False)
        ...
        # x_fusion = self.gat2_Fusion(x_fusion, edge_index_fusion)
        # x = global_mean_pool(batch_data.x, batch_data.batch).reshape(batch, x2, -1)
...
class RANF_Graph21(nn.Module):
    ...
    def forward(self, ret_specs_db, ret_itds, tgt_loc, tgt_sidx, ret_sidxs):
        ...
        for i in range(batch):
            ...
            graph_list.append(graph)
        batch_data = Batch.from_data_list(graph_list)
        batch_data = batch_data.to(ret_specs_db.device)
        x_hrtf, edge_index_hrtf = batch_data.x, batch_data.edge_index
        x_hrtf = self.gat1_hrtf(x_hrtf, edge_index_hrtf)
        x_hrtf = F.elu(x_hrtf)
        x_hrtf = self.gat2_hrtf(x_hrtf, edge_index_hrtf)
        ...
        x_fusion = F.elu(x_fusion)
        x = global_mean_pool(x_fusion, batch_data.batch).reshape(batch, x2, -1)
        x_spec = global_mean_pool(x_hrtf, batch_data.batch).reshape(batch, 16, -1)


        x_itd1, x_itd2, _ = x[:, :1, :], x[:, -1:, :], x[:, 1:-1, :]
        # Spectra post-processing
        for n in range(self.spec_hidden_layers):
            u, v = self._compute_uv(tgt_onehot[:, :1, :], None, n + self.hidden_layers, "target")
            x_spec = self.spec_hidden_blocks[n](x_spec, u=u, v=v)
